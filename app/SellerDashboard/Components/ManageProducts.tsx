...
export default function ManageProducts() {
  ...
  <div className='w-full h-screen flex items-start flex-col px-2'>
    ...
    <input type="text" placeholder='Search' className='bg-white outline-none rounded-md w-full px-2 mt-2 h-14'/>

    <div className="w-full flex items-center gap-x-2 pb-4 pt-2 overflow-x-auto overflow-y-hidden">  
    <OverallStorage
    className='select border-slate-950 text-black font-bold bg-white mt-3 font-abc'
    onChange={(e)=> handleSelectChange("OverallStorage", e.target.value)}
    value={Selected.OverallStorage || ''}
    />

    <InStock
    className="select border-slate-950 text-black font-abc font-bold bg-white mt-3" 
    ...
    <Draft
    className="select border-slate-950 font-abc font-bold text-black bg-white mt-3" 
