...
export default function Schedule() {
    ...
    const { DateSchedule, setDate } = useSchedule()
  ...
  return (
    <div className=''>
        ...
        <div className='pt-4 pr-5'>
            ...
            <input type="date" className="mt-1 w-full px-3 py-2 border rounded-lg text-white focus:outline-none focus:ring-2 focus:ring-blue-500"/>
        </div>

        {/* Time Picker */}
        <div className='pt-4 pr-5'>
            <label className="block text-white">Select Time</label>
            <input type="time" className="mt-1 w-full px-3 py-2 border rounded-lg text-white focus:outline-none focus:ring-2 focus:ring-blue-500" value={DateSchedule} onChange={(e: React.ChangeEvent<HTMLInputElement>)=> setDate(e.target.value)}/>
        </div>
    </div>
  )
}
